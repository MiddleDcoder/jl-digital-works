...
const navLinks = [
  ...
  { href: '#solution', label: 'Services' },
  {
    label: 'More Services',
    dropdown: [
      { href: '#automation', label: 'GHL & Automation' },
      { href: '#tracking', label: 'Tracking & Analytics' }
    ]
  },
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? 'bg-background/90 backdrop-blur-lg shadow-sm border-b border-border' : 'bg-transparent'
      }`}
    >
      <nav className="container-custom mx-auto px-4 md:px-8 py-4">
        <div className="flex items-center justify-between">
          ...
          <a href="#hero" className="flex items-center gap-2">
            <img src={logo} alt="JL Digital Works Logo" width="40" height="40" loading="eager" className="w-10 h-10" />
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              href="#contact"
              className="hidden sm:inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-primary-foreground rounded-full font-semibold text-sm hover:opacity-90 transition-all"
            >
              Get Started
          ...
          <div className="md:hidden absolute top-full left-0 right-0 bg-background border-b border-border shadow-lg animate-fade-in">
            <div className="flex flex-col py-4 px-4">
              {navLinks.map((link) =>
                'dropdown' in link ? (
                  <div key={link.label} className="flex flex-col">
                    <button
                      onClick={() => setDropdownOpen(!dropdownOpen)}
                      className="py-3 px-4 text-foreground font-medium hover:bg-secondary rounded-lg transition-colors flex items-center justify-between"
                    >
                      {link.label}
                      <ChevronDown className={`w-4 h-4 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} />
                    </button>
              ...
              <a
                href="#contact"
                onClick={() => setIsOpen(false)}
                className="mt-4 py-3 px-4 bg-primary text-primary-foreground rounded-lg font-semibold text-center hover:opacity-90 transition-all flex items-center justify-center gap-2"
              >
                Get Started
                <ArrowRight className="w-4 h-4" />
              </a>
